...
- Use hyphens to separate words (`config-parse`, not `config_parse`).
...
(a branded solution for a specific domain, similar to how `axum` is a web framework):
...
**When the convergence primarily wraps well-known external ecosystem crates**
(tokio, hyper, serde, etc.):
...
Trait-definition crates and their implementations follow a base-suffix pattern:
